...
public class ClienteDAOImpl implements ClienteDAO {
	...
	public int insertClienteAutoId(Cliente c) {
		// TODO Auto-generated method stub
		SimpleJdbcInsert jdbcI = new SimpleJdbcInsert(jdbc)
		...
		parametros.put("s_nombre", c.getNombreCompleto());
		parametros.put("s_apellidos", c.getSapellidos());
		parametros.put("s_nacimiento", c.getFnacimiento());
		parametros.put("b_activo", c.getBactivo());
		
		Number id_generated = jdbcI.executeAndReturnKey(parametros);
	...
	public void updateCliente(Cliente c) {
		...
		Object[] parametros = new Object[] {
			c.getSnombres(),c.getSapellidos(),c.getFnacimiento(),c.getBactivo(),c.getCcliente()
		};
	...
	public int ejecutarProcedimientoJdbc(Integer cliente, Boolean estado) {
		SimpleJdbcCall jdbcCall = new SimpleJdbcCall(jdbc)
				.withSchemaName("store")
				.withProcedureName("sp_actualizar")
	...
	public int[][] batchInsertVehiculos(List<Vehiculo> vehiculos) {
		...
		int[][] resultado = jdbc.batchUpdate(sql, vehiculos, 1000, new ParameterizedPreparedStatementSetter<Vehiculo>() {
